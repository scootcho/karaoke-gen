...
# NOTE: This Python code is also available in the karaoke-gen wheel at
# backend/services/gce_encoding/main.py. A future optimization would be to
# run `uvicorn backend.services.gce_encoding.main:app` instead of writing
# main.py here. For now, keeping the heredoc for incremental rollout.
