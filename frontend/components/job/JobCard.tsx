      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-1">
            <span className="text-slate-600">ID:</span> {job.job_id} <span className="text-slate-600">•</span> {createdAt}
          </p>
        </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        {isInteractive && (
          autoModeEnabled ? (
            ...
            <>
              {(job.status === "awaiting_review" || job.status === "in_review") && (
                <a
                  href={getReviewUrl()}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-orange-600 hover:bg-orange-500 text-white"
                >
              ...
              {job.status === "awaiting_audio_selection" && (
                <Button
                  ...
                  className="text-xs h-7 px-2 bg-blue-600 hover:bg-blue-500"
                >
                  Select Audio
                </Button>
              )}
              {job.status === "awaiting_instrumental_selection" && (
                <Button
                  ...
                  className="text-xs h-7 px-2 bg-pink-600 hover:bg-pink-500"
                >
                  Select Instrumental
                </Button>
              )}
        ...
        {/* Standard actions (logs, cancel, delete, retry) */}
        <JobActions
          job={job}
          onRefresh={onRefresh}
          showLogs={showLogs}
